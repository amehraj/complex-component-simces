    ...
    async def process_epoch(self) -> bool:
        ...
        if self._current_input_components:
            if self._complex_string == "Correct":
                self._current_number_sum = self._current_number_sum + self._complex_value
            else:
               self._current_number_sum = self._current_number_sum - self._complex_value
        else:
            self._current_number_sum = self._complex_value + self._latest_epoch / 1000
...
def create_component() -> ComplexComponent:
    ...
    complex_string = cast(str, environment_variables[COMPLEX_STRING])
