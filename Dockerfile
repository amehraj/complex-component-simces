...
FROM python:3.7.9
LABEL org.opencontainers.image.source https://github.com/simcesplatform/complex-component
LABEL org.opencontainers.image.description "Docker image for the complex-component for the SimCES platform."

# create the required directories inside the Docker image
RUN mkdir -p /complex_component
RUN mkdir -p /init
RUN mkdir -p /logs
...
COPY complex_component/ /complex_component/
COPY init/ /init/
...
CMD [ "python3", "-u", "-m", "complex_component.complex_component" ]